...
#let __conditional_show(body, rule: it => it, condition: true) = {
  if condition {
    rule(body)
  } else {
    body
...
#let project(title: "Typst Book", description: none, authors: (), kind: "page", plain-body) = {
  ...
  set text(
    // font: main-font,
    size: if not is-pdf-target { main-size } else { 11pt },
    fill: main-color,
    lang: "en",
  )

  // markup setting
  show: __conditional_show.with(
    rule: markup-rules.with(..common, dash-color: dash-color),
    condition: is-web-target,
  )
  // math setting
  show: __conditional_show.with(
    rule: equation-rules.with(..common, theme-box: theme-box),
    condition: is-web-target,
  )
  // code block setting
  show: __conditional_show.with(
    rule: code-block-rules.with(..common, themes: themes, code-font: code-font),
    condition: is-web-target,
  )
